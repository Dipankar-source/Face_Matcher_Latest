...
# --- TOP: UPLOADER ---
st.markdown(
    "<h2 style='text-align: center; font-weight: 300; letter-spacing: 2px;'>"
    "B I O M E T R I C &nbsp; S C A N N E R</h2>",
    unsafe_allow_html=True
)
...
with col_visuals:
    ...
    with v_col2:
        ...
        if match_img_path:
            ...
            st.image(
                "https://via.placeholder.com/300x300.png?text=No+Match",
...
with col_controls:
    ...
    st.markdown(
        "<p style='color: #aaa; margin-bottom: 0;'>Identity:</p>",
        unsafe_allow_html=True
    )
    st.markdown(
        f"<h1 style='margin-top: 0;'>{name}</h1>",
        unsafe_allow_html=True
    )
    
    st.markdown(
        "<p style='color: #aaa; margin-bottom: 0;'>Confidence:</p>",
        unsafe_allow_html=True
    )
    st.markdown(
        f"<div class='metric-box' style='color:{match_color}'>{score:.1%} "
        f"<span style='font-size: 1rem; color:white; border:1px solid {match_color}; "
        f"padding: 2px 8px; border-radius: 4px;'>{status_text}</span></div>",
        unsafe_allow_html=True
    )
    ...
    with st.form("fix_identity"):
        new_name_input = st.text_input(
            "📝 Correction / New Name",
            placeholder="Enter correct name here..."
        )
